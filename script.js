...
function displayDegrees(){
    ...
    fetch('Iftikhar_info.json').then((response) =>{
        ...
        for (let i = 0 ; i< data.Degrees.length; i++){
            ...
            var node = document.createTextNode("Program : " + data.Degrees[i].degree.Program);
            para1.appendChild(node);
            element.appendChild(para1);

            var node = document.createTextNode("Type : " + data.Degrees[i].degree.Type);
            para2.appendChild(node);
            element.appendChild(para2);

            var node = document.createTextNode("Year : " + data.Degrees[i].degree.Year);
